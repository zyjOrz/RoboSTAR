...
class RoboSTARVocabulary:
    """mT5 tokenizer extended with atomic motion-code symbols.

    Motion symbols use the RoboSTAR namespace. They are added to the base
    ...
    def __init__(
        ...
        language_token = "<robostar:en_asl>"
        body = [f"<robostar:body:{index}>" for index in range(self.cfg.body_codes)]
        left = [f"<robostar:left:{index}>" for index in range(self.cfg.hand_codes)]
        right = [f"<robostar:right:{index}>" for index in range(self.cfg.hand_codes)]
...
def token_contract() -> dict[str, Any]:
    return {
        "version": "robostar_fsq_c2f_mt5_single_code_v1",
        "frame_dim": 133,
        "body_dim": 43,
        "left_dim": 45,
        "right_dim": 45,
