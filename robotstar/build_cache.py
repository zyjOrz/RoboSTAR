...
def main() -> None:
    ...
    tokenizer = RobotSTARTokenizer.from_pretrained(args.tokenizer_model, device=device).eval()
    vocab = RobotSTARVocabulary(args.base_model, 96, 192, True)
    ...
    for split in [value.strip() for value in args.splits.split(",") if value.strip()]:
        ...
        torch.save(
            {
                "version": "robotstar_c2f_cache_v1",
                "rows": rows_out,
                "split": split,
